...
def load_filter_from_file(mydict, item_count):
    lfff_count = 0
    returntext = "\n"
...
def fetch_fps(mydict, file_path):
    if mydict["NoGap"]:
        a = 1;
    file_fps = []
...
def make_exo(objdict, mydict, file_path, file_fps):
    end = {}
    exo_result = "[exedit]\nwidth=" + str(1280) + "\nheight=" + str(720) + "\nrate=" + str(
        60) + "\nscale=1\nlength=99999\naudio_rate=44100\naudio_ch=2"
    item_count = 0
    ...
    for index in range(1, len(objdict["length"])):
        ...
        if obj_frame_pos < bf:
            bf = 0
            bfidx = index
            layer += 1 + mydict["SepLayerEvenObj"]
            if obj_frame_pos < 0:
                continue
        ...
        # ファイルから効果を読み込む設定
        if mydict["EffPath"] != "":
            a, b = load_filter_from_file(mydict, item_count)
            exo_eff += a
            lfff_count += b

        # 偶数番目オブジェクトをひとつ下のレイヤに配置する
        if mydict["SepLayerEvenObj"] == 1 and (bfidx + item_count) % 2 == 0:
        ...
        if mydict["OutputType"] != 3 and int(mydict["IsFlipHEvenObj"]) == 1 and (bfidx + item_count) % 2 == 0:
            ...
            if mydict["ScriptText"] != "":  # スクリプト制御追加する場合
                exo_script = ("\n[" + str(item_count) + "." + str(len(mydict["Effect"]) + 2 + lfff_count) +
                              "]\n_name=スクリプト制御\ntext=" + encode_exo(mydict["ScriptText"]))
                asc_count = 1
        ...
        # フィルタ効果  奇数番目（反転×）
            ...
            if mydict["ScriptText"].get('1.0', 'end-1c') != "":  # スクリプト制御追加する場合
                exo_script = ("\n[" + str(item_count) + "." + str(len(mydict["Effect"]) + 1 + lfff_count) +
                              "]\n_name=スクリプト制御\ntext=" + encode_exo(mydict["ScriptText"]))
                asc_count = 1
            exo_4_2 = "\ngroup=1\noverlay=1"
